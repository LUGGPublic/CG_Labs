...
int main()
{
    ...
    // Set up the scene graph by adding the planets to the Sun
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        while (!celestial_stack.empty()) {
            ...
            CelestialBodyRef current_ref = celestial_stack.top();
            celestial_stack.pop();  // Remove it from the stack
            
            // Render the current celestial body and get the transform matrix for its children
            glm::mat4 child_transform = current_ref.body->render(animation_delta_time_us,
                                                                 camera.GetWorldToClipMatrix(),
                                                                 current_ref.parent_transform,
                                                                 show_basis);
            
            // Push all children of the current body onto the stack, passing the current body's transform
            for (CelestialBody* child : current_ref.body->get_children()) {
                celestial_stack.push({child, child_transform});
            }
        }
