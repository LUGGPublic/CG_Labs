...
int main()
{
    ...
    OrbitConfiguration const sun_orbit{ 0.0f, 1.0f, 1.0f};
	...
	glm::vec3 const saturn_ring_scale{ 1.0f, 1.25f, 0.1f};
    ...
    CelestialBody saturn_ring(saturn_ring_shape, &celestial_body_shader, saturn_ring_texture);
    saturn_ring.set_scale(saturn_ring_scale);
	...
	while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 parent_transform = glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f));
        std::stack<struct CelestialBodyRef> bodies;
        struct CelestialBodyRef planets[10] = {{&sun},{&mercury},{&venus},{&earth},{&moon},{&mars},{&jupiter},{&saturn},{&uranus},{&neptune}};
        
        for(CelestialBodyRef planet : planets){
            CelestialBody *body = planet.body;
            std::vector<CelestialBody*> vec = (*body).get_children();
            
            if(vec.size() == 0)
                continue;
            parent_transform = (*body).render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
            for(CelestialBody* child : vec){
                (*child).render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
            }
        }
        
        /*parent_transform = sun.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = mercury.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = venus.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = earth.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = mars.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = jupiter.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = saturn.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        parent_transform = uranus.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
        neptune.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);*/
